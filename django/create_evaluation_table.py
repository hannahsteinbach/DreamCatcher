...
random_dreams = df.sample(n=3, random_state=42)
dream_data = []
dreamy_data = []
...
for dream in dreamy_data:
    metadata = zero_shot_prompting(dream)  # Assuming `add_metadata` is a static method in the Dream model
    csv_data = []
...
csv_file = "zero_shot_llama.csv"
with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
    writer = csv.DictWriter(file, fieldnames=['text', 'keywords', 'characters', 'places', 'emotion'])
    writer.writeheader()
    for row in csv_data:
        # Convert lists to string format before writing to CSV
        row['keywords'] = ', '.join(row['keywords'])
        row['characters'] = ', '.join(row['characters'])
        row['places'] = ', '.join(row['places'])
        writer.writerow(row)

print(f"Zero shot llama data written to {csv_file}")
...
def multi_shot_prompting(content_str):
    ...
    dream_prompt = (
        ...
        "- keywords: A list of at most 5 keywords extracted from the dream content, excluding variants of 'dream' and stop words. Keywords can also be a fixed expression (e.g. compound nouns).\n"
        "Only output the dictionary. Do not include any other information. All values should be on the same line. The keys should be in double quotes."
        "Here are 5 examples on how to annotate. You will receive the dream first, followed by a json object with the previously specified keys.\n"
...
print(f"Multi shot llama data written to {csv_file}")
